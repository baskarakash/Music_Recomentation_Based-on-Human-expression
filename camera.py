...
class VideoCamera(object):
	...
	def get_frame(self):
		...
		global age,emotion_dict,maxindex
		...
		for (x,y,w,h) in face_rects:
			...
			#roi_rgb_frame= image[y:y + h, x:x + w]
			cropped_img = np.expand_dims(np.expand_dims(cv2.resize(roi_gray_frame, (48, 48)), -1), 0)
			#cropped_img2=np.expand_dims(np.expand_dims(cv2.resize(roi_gray_frame, (200, 200)), -1), 0)
			#cropped_img2=cropped_img2/255
			prediction = emotion_model.predict(cropped_img)
			prediction2 = facecvi.detect_face(image)
			...
			#print(df1)
			age = int(prediction2)
			#age=cm_plot_labels[maxindex2]
			#gender = np.round(prediction2[1][0])
			#gender_type = 'male' if gender == 0 else 'female'
			text = "Age - "+str(age) #+ " Gender - " + gender_type 

			cv2.putText(image, text,(10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 1, cv2.LINE_AA)
		...
		return jpeg.tobytes(),age,emotion_dict[maxindex]
